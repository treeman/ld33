...
// TODO move to line?
// Only makes sense for FPoint's I think?
...
public:
    ...
    // Use FPoint's as an interesection can be in the middle.
    // Use is_on for regular check.
    vector<FPoint> intersect(const Point<T> &p) const {
        if (is_on(p)) {
            vector<FPoint> res = { FPoint(p) };
            return res;
        }
        return vector<FPoint>();
    }
    vector<FPoint> intersect(const LineSegment<T> &o) const {
        ...
        if (is_collinear(a, b, o.a) && is_collinear(a, b, o.b)) {
            ...
            if (fa > fb) swap(fa, fb);
            // Not overlapping.
            if (fb < 0.0 || fa > 1.0) return vector<FPoint>();

            // Find min/max points in both.
            FPoint d(o.b - o.a);
            FPoint x(FPoint(o.a) + d * max(fa, 0.0));
            FPoint y(FPoint(o.a) + d * min(fb, 1.0));
            ...
            // Smallest (x, y) first.
            if (y < x) swap(x, y);
            vector<FPoint> res = { x, y };
            return res;
        }

        Line l1(a, b), l2(o.a, o.b);
        // If parallel, no intersection.
        if (l1.is_parallel(l2)) return vector<FPoint>();
        FPoint p = l1.intersect(l2);

        // Handle the segment part of line intersection. Should be on both.
        FPoint da(a), db(b), doa(o.a), dob(o.b);
